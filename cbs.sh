...
execute_with_timeout() {
    ...
    timeout 2 $command_to_run
    cmd_status=$?

    # Print messages with colors
    if [ $cmd_status -eq 124 ]; then
        echo -e "\033[0;31mTimeout! Command '$command_to_run' was forcibly terminated.\033[0m"
        echo "Timeout! Command '$command_to_run' was forcibly terminated." >> cbs_result.txt
        ...
        echo "Unexpected termination! Command '$command_to_run' exited with status $cmd_status." >> cbs_result.txt
    elif [ $cmd_status -eq 0 ]; then
        echo -e "\033[0;32mCommand '$command_to_run' successfully completed.\033[0m"
        echo "Command '$command_to_run' successfully completed." >> cbs_result.txt
    fi
}

# 여기에 세빈이형 테스트 내용 넣으면 돼요
ENVIRONMENTS=("HB" "RN" "WH")
ROBOTNUM=(5 10 15 20 25 30)
MAXCOUNT=50

BASE_COMMAND="python centralized/cbs-ta/cbs-ta.py"
