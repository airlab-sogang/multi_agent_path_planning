        ...
        while open_set:
            ...
            if min_f_score < new_min_f_score:
                for open_node in open_set:
                    if w * min_f_score < f_score[open_node] <= w * new_min_f_score:
                        focal_set |= {open_node}
                min_f_score = new_min_f_score

            # select current node from FOCAL
            temp_dict = {
                focal_item: (d_score.setdefault(focal_item, float("inf")), f_score.setdefault(focal_item, float("inf")))
            ...
            for neighbor in neighbor_list:
                ...
                if neighbor not in open_set:
                    open_set |= {neighbor}
                    if tentative_g_score + self.admissible_heuristic(neighbor, agent_name) <= w * min_f_score:
                        focal_set |= {neighbor}
                ...
                d_score[neighbor] = d_score[current] + self.focal_vertex_heuristic(neighbor) + self.focal_edge_heuristic(current, neighbor)
